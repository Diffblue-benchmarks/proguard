...
class LibraryKeepCheckerDiffblueTest {
  /**
   * Method under test: {@link LibraryKeepChecker#checkClassSpecifications(List)}
   */
  @Test
  void testCheckClassSpecifications() throws IOException {
    ...
    LibraryKeepChecker libraryKeepChecker = new LibraryKeepChecker(programClassPool, libraryClassPool,
        new WarningPrinter(new PrintWriter(new StringWriter())));

    // Act
    libraryKeepChecker.checkClassSpecifications(new ArrayList<>());
